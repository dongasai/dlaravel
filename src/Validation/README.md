...
创建一个新的class，并继承 `\Ucore\ValidationCore`。用于一个（或一系列相关）请求的验证, 相当于 laravel 的 表单请求验证
...
class PageRequestValidation extends \Ucore\ValidationCore
{
...
```php
// 验证 POST 数据
$v = PageRequestValidation::check($_POST);
...
- 一个函数名称
- 一个当前验证类的方法名
    - 在继承了 `Ucore\Validation` 的子类添加验证方法. 请看上面的 [使用方式1](#how-to-use2)
...
> 别忘了继承 `\DLaravel\Validator`,和实现必须方法`validate`

```php
class AdemoValidator extends \DLaravel\Validator
{
