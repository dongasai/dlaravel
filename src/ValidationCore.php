...
use DLaravel\Validator\PrepareValidator;
...
abstract class ValidationCore extends Validation
{
    ...
    /**
     * 使用Protobuf-Message创建验证器
     * @param \Google\Protobuf\Internal\Message $message
     * @param $scene
     * @return static
     */
    public static function makeByProrobuf(\Google\Protobuf\Internal\Message $message, $scene = ''): static
    {
        return new static(json_decode($message->serializeToJsonString(),true), [], [], $scene);
    }
    ...
    public function validated(): static
    {
        ...
        if ($this->isFail()) {
            throw new Exception\ValidateException($this, $this->firstError());
        }
    ...
    protected function callCatsRules(array $rules): array
    {
        if ($this->cats) {
            foreach ($this->cats as $name => $class) {
                $rules[] = [
                    $name, new EnumValidator($this, [ $class ])
                ];
            }
        }
